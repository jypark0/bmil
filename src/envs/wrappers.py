...
class ObservationAndGoalWrapper(gym.ObservationWrapper):
    def __init__(self, env):
        super().__init__(env)
        self.observation_space = spaces.Dict(
            [
                ("observation", env.observation_space.spaces["observation"]),
                ("desired_goal", env.observation_space.spaces["desired_goal"]),
            ]
        )
...
# Gym 0.14.0
# https://github.com/openai/gym/blob/0.14.0/gym/wrappers/dict.py
class FlattenDictWrapper(gym.ObservationWrapper):
    ...
    def get_box_size(self, space):
        ...
        ), "Only spaces of type Box are supported."
